...
class DevinApiClient:
    ...
    def build_pr_analysis_prompt(
        ...
        return (
            ...
            f"\"DevClean fix for pr #{pr_number} - [one-line description]\"\n"
            f"  where [one-line description] is a "
            f"concise summary of the fix.\n"
            f"- Structure the PR body using this exact format. "
            f"Replace the entire PR body with exactly this content — "
            f"do not include any pre-filled PR template sections "
            f"(e.g. checklists, 'ADDITIONAL INFORMATION', or any other "
            f"template placeholder text):\n"
            f"\n"
            f"---\n"
            f"## [DevClean fix] Found [category] in pr #{pr_number} - [category] - [one-line description]\n"
            f"\n"
            ...
            f"Format the category as Title Case with spaces instead of underscores "
            f"(e.g. INCONSISTENT_PATTERNS → Inconsistent Patterns, "
            f"INCOMPLETE_ERROR_HANDLING → Incomplete Error Handling, "
            f"COMPLEX_DEAD_CODE → Complex Dead Code, "
            f"DOC_DRIFT → Doc Drift):]\n"
            f"- [Fix 1](#fix-1-category--short-description): [Category Title Case] - [short description]\n"
            f"- [Fix 2](#fix-2-category--short-description): [Category Title Case] - [short description]\n"
            f"(and so on for each fix)\n"
            f"\n"
            f"---\n"
            f"\n"
            f"[Then, for each fix, add a full details section with this structure. "
            f"Use the same Title Case category format for the heading:]\n"
            f"\n"
            f"### Fix [N]: [Category Title Case] - [short description]\n"
            f"\n"
            f"- **File:** `[file path]`\n"
            ...
            f"falls under and its impact on the codebase, and how the fix addresses it]\n"
            f"\n"
            f"---\n"
            f"\n"
